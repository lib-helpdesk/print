...
var simulationMode = false; // Simulation mode toggle

$(function() { //onload, runs when page finishes loading
	// Add simulation mode toggle for debugging
	var simulationToggle = $("<div>")
		.attr('class', 'debug-controls')
		.css({
			'position': 'fixed',
			'top': '10px',
			'right': '10px',
		...
		.html("Enable Simulation Mode")
		.click(function() {
			simulationMode = !simulationMode;
			$(this).html(simulationMode ? "Disable Simulation Mode" : "Enable Simulation Mode");
			fetchDeviceStatus();
		});
	$("body").append(simulationToggle);
...
function fetchDeviceStatus() {
	...
	for (var i=0; i<printers.length; i++) {
		...
		//if ((printer.type == "HP806") || (printer.type == "HP651")) {
	...
	for (var i=0; i<printers.length; i++) {
		if (printers[i]["error"]) {
			console.log('Error detected for printer: ' + printers[i].name);
		}
	}
}
...
function fetchDeviceStatusHPM806(printer) {
	...
	var requestData = {url:urlDeviceStatus, printer:printer, rnd:Math.random()};
	if (simulationMode) {
		requestData.simulate = true;
	}
	
	$.post("fetchUrl.php?rnd="+Math.random(), requestData, function(data) { //if the printer responds to the HTTP request, this function will run
	//data is the HTML generated by the device status page by way of fetchUrl.php
		
		console.log("Starting \t" + printer.name);
		console.log("Response data length: " + data.length);
		
		// Debug the first 100 characters of the response
		console.log("Response preview: " + data.substring(0, 100));
		
		// Check if we got fallback data
		if (data.indexOf("Unable to reach printer") > -1) {
			console.log("Got fallback data for printer: " + printer.name);
		}
		...
		console.log("Status found: " + (currentStatus || "Not found"));
